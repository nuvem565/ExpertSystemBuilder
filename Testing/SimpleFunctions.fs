...
let defuzzify (qualifier:Qualifier) (inputValues: (string * float)list) =
    ...
    // debug
    (*[
        for center in centersOfSets do
            match List.tryFind (function name, _ when name = fst(center) -> true | _ -> false) inputValues with
            | Some(name, valueY) -> yield (fst(center), name, snd(center), valueY)
            | _ -> yield " ", " ", 0.0, 0.0
    ],[
        for inputY in inputValues do
            match List.tryFind (function center, _ when center = fst(inputY) -> true | _ -> false) centersOfSets with 
            | Some(name, valueX) -> yield (name, fst(inputY), valueX, snd(inputY))
            | _ -> yield " ", " ", 0.0, 0.0
    ]*)
