...
let isEnumDefined key str = 
    if isQualifier key then
        ...
        | Some items -> List.exists (fun item -> item = str) items
        | None -> false
    else false
let inRange key f =
...
let CSVget a b c = 
    ...
    csvToNumber a b c (delimiter)

let CSVset a b c e = 
    let (CSV delimiter) = basicInfo.CSVDelimiter
    saveToCsv a b c delimiter e
...
let evalOperation operation firingLevel = 
    ...
    //| Clear variables, qualifiers, choices -> 
...
let interpretRule (r:Rule) = 
    ...
    if ifFiringLevel > fuzzyThreshold then
        ...
        evalElse r // if rule is not on "Selected" list
        if basicInfo.Derivation <> Derivation(Derivation.AllRulesUsed) then setRule r ifFiringLevel
...
let rec sumOfFuzzyValues valuesWithSetTruthness (vals:(string) list) = // It could be done better - without Some _ option in lookUpQualifier
    ...
    | value :: rest -> 
        ...
        | [] -> 0.
        | list -> 
            match List.tryFind (function valXmemFunc -> fst(valXmemFunc) = value) list with
...
lookUpQualifier <-
    fun key vl ->
        ...
        | true, Some valuesWithTruthness -> //if it's infered by now so it only checks it value(s)
...
let defuzzifiedQualifiers = executeProgram ()
...
(*
// Test for eval and evalBool functions
let evalTest expr str solution = 
    let result = eval expr
    printfn "Expression: %s = %A" str result
    printfn "Test result: %A" (result =~ solution)
    printfn "---------"

let evalBoolTest expr str = 
    let result = evalBool expr
    printfn "Bool expression: %s" str 
    printfn "Test result: %A" result
    printfn "---------"
    
// Test expressions:
let str1 = "0+10*(1*9/3-5 *15)"
let expr1 = Expr("+",Expr.Const 0.0, Expr("*",Expr.Const 10.0, Expr("-",Expr ("/",Expr ("*",Expr.Const 1.0,Expr.Const 9.0),Expr.Const 3.0),Expr ("*",Expr.Const 5.0, Expr.Const 15.0))))
let solution1 = -720.0
// Numeric variables test
let str2 = "0+10*(1*9/3-5*[ONPR])"
let expr2 = Expr("+",Expr.Const 0.0, Expr("*",Expr.Const 10.0, Expr("-",Expr ("/",Expr ("*",Expr.Const 1.0,Expr.Const 9.0),Expr.Const 3.0),Expr ("*",Expr.Const 5.0, Variable "OPNR"))))
let solution2 = -470.0
// to the power test
let str3 = "0^1+10^1^2*(1*9/3^0.5-5*[ONPR])"
let expr3 = Expr("+",Const 0.0, Expr("*",Expr("^", Const 10.0, Expr("^", Const 1.,Const 2.)), Expr("-",Expr ("/",Expr ("*",Const 1.0,Const 9.0),Expr("^",Const 3.0, Const 0.5)),Expr ("*",Expr.Const 5.0, Variable "OPNR"))))
let solution3 = -448.038475773 // still different to -448.0384758 (calculated) with epsilon*50000
// natural logarithm + scientific notation + base of natural logarithm + two arguments logarithm + tangent function + factorial of 5
let str4 = "ln 2 ^9 + 0.4*2 + log sin E tan 0.5 + 5!"
let expr4 = Expr("+",Expr("+",Expr("+",Expr("^", Prefix("ln", Const 2.), Const 9.), Expr("*",Const 4e-1, Const 2.)),Expr("log", Prefix("sin", Const Math.E), Prefix("tan", Const 0.5))),Prefix("!", Const 5.))
let solution4 = Math.Pow((Math.Log 2.), 9.) + 4e-1*2.0 + Math.Log(Math.Tan 0.5, Math.Sin (Math.E)) + factorial 5.
// Test expressions which returns bool
let boolExpr1 = Logical ("AND", Logical ("AND", BoolExpr.StringComparison(StringVar "CODE",StringConst "POCL"),Comparison("=",BoolLeaf (Expression (Variable "I")), BoolLeaf (Expression (Variable "NP")))), QualifierComparison("Stan powierzchni:",["surowy (przed obrobka)",0.7]))
let boolStr1 = """
Based on rule 43:
[CODE]="POCL"
and:   [I]=[NP]
and:   Stan powierzchni: NOT {surowy (przed obrobka)}
"""
let boolExpr2 = Logical("AND",Logical("AND",QualifierComparison ("Postac polfabrykatu:", ["pret walcowany", 0.3; "pret ciagniony", 0.6]),Comparison (">",BoolLeaf (Expression (Variable "LPF")),BoolLeaf (Expression (Variable "LPO")))), Comparison(">",BoolLeaf (Expression (Const 0.0)), BoolLeaf (Expression (expr2))))
let boolStr2 = """
Based on rule 20:
Postac polfabrykatu: {pret walcowany} OR {pret ciagniony}
and:   [LPF]>[LPO]
and:   0.0 > 0+10*(1*9/3-5*[ONPR])
"""

evalTest expr1 str1 solution1 |> ignore
evalTest expr2 str2 solution2 |> ignore
evalTest expr3 str3 solution3 |> ignore
evalTest expr4 str4 solution4 |> ignore
//evalBoolTest boolExpr1 boolStr1 |> ignore
//evalBoolTest boolExpr2 boolStr2 |> ignore
*)

//printfn "New CSV: %A" (CSVget (__SOURCE_DIRECTORY__ + "\\example.csv") 3. 2.)
//printfn "New CSV: %A" (CSVset (__SOURCE_DIRECTORY__ + "\\example.csv") 3. 2. 30040.)
//printfn "New CSV: %A" (CSVget (__SOURCE_DIRECTORY__ + "\\example.csv") 3. 2.)
