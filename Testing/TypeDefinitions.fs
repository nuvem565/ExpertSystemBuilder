...
let String50 s = 
    if System.String.IsNullOrEmpty s || s.Length > 50
    then None
    else String50 s |> Some

let example1 = String50 "akgawrg"
*)
...
type GeneralVariable = 
    | StringV of string
    | NumericV of string
    | QualifierV of string // name parameter from qualifier resize array record
*)
...
type Operations =
    | Report of string
    | Clear of Terminal list // variables, qualifiers, choices
    | Save of Terminal * Expr * Expr * Terminal // path to CSV, row, column, expression to save
    | Delay of Terminal * Terminal
    | Read of Terminal * Expr * Expr * Terminal // path to CSV, row, column, variable to overwrite by read value
    | Defuzzify
    | AssignChoice of string * value:float
    | AssignString of var:string * strExpr:Terminal list
    | AssignNumeric of var:string * expr:Expr
    | AssignQualifier of string * (string * float) list
...
type Rule =
    {
        ...
        Number: int32
        IfClauses: BoolExpr
        ThenStatements: Operations list
        ElseStatements: Operations list option
        State: RuleValue
    }
    
type Rules = Rules of ResizeArray<Rule>

type Section =
    | Basic of BasicInfo
    | Qualifiers of ResizeArray<Qualifier>
    | Choices of ResizeArray<Choice>
    | Variables of ResizeArray<Variable>
    | Rules of ResizeArray<Rule>
